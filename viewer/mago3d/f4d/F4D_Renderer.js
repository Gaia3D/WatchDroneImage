...
f4d_renderer.prototype.render_F4D_neoRefLists = function(GL, neoRefList_array, neoBuilding, f4d_manager, isInterior, standardShader, renderTexture)
{
	// render_neoRef
...
f4d_renderer.prototype.forceRender_F4D_neoRefLists = function(GL, neoRefList_array, neoBuilding, f4d_manager, isInterior, standardShader, renderTexture)
{
	...
	for(var j=0; j<neoRefLists_count; j++)
	{
		...
		for(var k=0; k<visibleIndices_count; k++)
		{
			...
			///var neoReference = neoRefList.neoRefs_Array[neoRefList._currentVisibleIndices[k]]; // TEST.***
			if(!neoReference || neoReference== undefined)
			{
				continue;
			}
...
f4d_renderer.prototype.render_F4D_simpleBuilding_V1_PostFxShader = function(GL, BR_Project, f4d_manager, imageLod, shader)
{
	...
	//for(var i=0; i<simpObjs_count; i++)
	{
		...
		//for(var k=0; k<vt_arraysCacheKeys_arrays_count; k++)
		{
			...
			GL.vertexAttribPointer(shader.texCoord2_loc, 2, GL.UNSIGNED_SHORT, true,20,12);
...
f4d_renderer.prototype.render_F4D_simpleBuilding_Normal_V1 = function(GL, BR_Project, modelViewProjRelToEye_matrix, encodedCamPosMC_High, encodedCamPosMC_Low, f4d_manager, imageLod)
{
	var simpBuildV1 = BR_Project._simpleBuilding_v1;
	//var simpObjs_count = simpBuildV1._simpleObjects_array.length;
	var f4d_shadersManager = f4d_manager.f4d_shadersManager;
	...
	//for(var i=0; i<simpObjs_count; i++)
	{
		...
		//for(var k=0; k<vt_arraysCacheKeys_arrays_count; k++)
		{
			...
			*/
			//-------------------------------------------------------------------------------------------------------------------------------------
			GL.bindBuffer(GL.ARRAY_BUFFER, this.simpObj_scratch._vtCacheKeys_container._vtArrays_cacheKeys_array[0]._verticesArray_cacheKey);
			GL.vertexAttribPointer(shader._position, 3, GL.FLOAT, false,16,0);
			GL.vertexAttribPointer(shader._texcoord, 2, GL.UNSIGNED_SHORT, true,16,12);
			
			GL.bindBuffer(GL.ARRAY_BUFFER, this.simpObj_scratch._vtCacheKeys_container._vtArrays_cacheKeys_array[0]._normalsArray_cacheKey);
			GL.vertexAttribPointer(shader._normal, 3, GL.BYTE, true,3,0);
